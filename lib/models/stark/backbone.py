...
from lib.models.stark.clipvit16 import CLIPVIT
...
def build_backbone_simtrack(cfg):
    net_type = cfg.MODEL.BACKBONE.TYPE
    num_features = cfg.MODEL.BACKBONE.NUM_FEA
    template_sz = cfg.MODEL.BACKBONE.TEMPLATE_SZ
    search_sz = cfg.MODEL.BACKBONE.SEARCH_SZ
    window_sz = cfg.MODEL.BACKBONE.WINDOW_SZ
    foveal_sz = cfg.MODEL.BACKBONE.FOVEAL_SZ
    backbone = CLIPVIT(num_features=num_features, template_sz=template_sz, search_sz=search_sz, window_sz=window_sz, foveal_sz=foveal_sz, net_name=net_type)
    return backbone
